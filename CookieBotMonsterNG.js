	...
	if(!skether.buildingIntervalIsRunning){
		...
		skether.buildingInterval = setInterval(function(){
			...
			if(document.getElementById("productPrice" + skether.prodID).style.color == "rgb(0, 255, 0)"){
				...
				else if(skether.justAscended && Game.buyBulk == 100)
				{
...
skether.afterAscending = function(){
	...
	setTimeout(function(){document.getElementById("upgrade0").click(); setTimeout(function(){skether.santa();}, 12500);}, 60000);
}

skether.checkAscension = function(){
	if(Game.ascendNumber.style.display == "block" && skether.canAscend && (Game.ascendMeterPercent > 0.97 || skether.fast)){
